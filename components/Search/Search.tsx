...
const Search: FC<Props & React.HTMLAttributes<HTMLDivElement>> = (props) => {
  ...
  return (
      <div className={cn('relative flex items-center ', otherProps?.className)}>
          ...
          <input
              className="bg-fillBackground w-full flex-1 rounded-[4px] outline-none pl-9 py-3 pr-10 text-[12px] leading-[20px] tracking-[0.96px] text-fontPrimary font-PPSupplySans"
              type="text"
          ...
          {searchTerm && (
              <IconX
                  className="absolute z-10 right-4 cursor-pointer text-fontPrimary"
                  size={18}
                  onClick={clearSearch}
              />
          )}
